...
import { UPDATE_RESERVATION, RESET_RESERVATION } from "../reducers/reservation-reducer";
...
function ReservationPage() {
  const { selectedPlateau } = useParams(); // depuis la page plateau 
  ...
  useEffect(() => { // a verifier
    if (selectedPlateau) {
      setPlateau(selectedPlateau);
      dispatch({
        type: UPDATE_RESERVATION,
        payload: { name, plateau: selectedPlateau, equipment, day, startTime, endTime },
      });
    }
  }, [selectedPlateau]);
  ...
  const handleReservation = async () => {
    ...
    if (!name || !plateau || !day || !startTime || !endTime) { // il peut y avoir aucun items 
  ...
  if (error) return <div>Error: {error}</div>; // le error du useReservationSystem 
